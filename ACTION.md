...
| `low-priority-paths` | no | (unset) | OPTIONAL. Comma-separated globs for reference/scratch paths (e.g. design mockups, one-off scripts). Findings there are capped at MEDIUM severity instead of CRITICAL/HIGH, not excluded. Extends GSR's built-in defaults (`design_prd/**`, `**/*.mockup.html`); can't remove them. Root-level `*.sh` is deliberately NOT a default (too broad — would dampen `build.sh`/`deploy.sh`-style CI/CD entry points); opt in with a narrower pattern for your own repo's scratch-script convention if useful. |
