...
## 📌 Project Overview
GSR (Gemini Subagent Reviewer) is an AI-powered code review tool designed to analyze code and provide rich feedback utilizing Gemini subagents. The project is split into two distinct architectures:

1. **GSR Local CLI Extension (`/gemini-cli-extension`)**
...
2. **GSR ADK Cloud Run Prototype (`/adk`)**
...
- **Module Systems**: Be aware of the varying module systems. `gemini-cli-extension` and `adk/frontend` use `"type": "module"` (ESM), whereas `adk/backend` defaults to CommonJS (`"type": "commonjs"`). Always use the appropriate import/export syntax for the respective directory.
...
1. **Strong Typing**: When working in TypeScript directories (`adk/backend`, `gemini-cli-extension/src`), always define clean, strict types or interfaces for API payloads, tool outputs, and configurations.
